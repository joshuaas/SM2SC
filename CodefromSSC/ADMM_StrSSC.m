...
function C2 = ADMM_StrSSC(Y, outliers, affine, alpha, Theta, gamma0, nu,  maxIter, Z, thr, beta, sum_ex,P,A)
if (nargin < 2)
...
%         A = inv(mu1*(P'*P)+mu2*eye(N+D));
        ...
        while ( (err1(i) > thr1 || err2(i) > thr2) && i < maxIter )
         ...
         %C2                      = max(0,(abs(Z                     +Lambda2          /mu2) - 1/mu2* ones(N+D,N)))                                              .* sign(Z           +Lambda2         /mu2);        
            ...
            C2(1:N,:) = max(0,(abs( Z(1:N,:)+Lambda2(1:N,:)/mu2) - 1/mu2*(ones(N,N)./nu + nu*gamma0*(1-Theta)...
                        + beta*sum_ex))).*sign(Z(1:N,:)+Lambda2(1:N,:)/mu2);
